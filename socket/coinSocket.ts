...
import { fetchCoinData } from '../service/coinService';
import logger from '../log/logger';

// 소켓 연결 설정 함수
export const coinSocket = (io: SocketIOServer): void => {
    io.on('connection', (socket: Socket) => {
        ...
        socket.on('subscribeToCoin', (coinId: string) => {
            ...
            const fetchDataAndEmit = async () => {
                try {
                    ...
                    logger.error(`Error fetching coin data for ${coinId}: ${(err as Error).message}`);
                }
            };
